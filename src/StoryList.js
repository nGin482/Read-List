    ...
    if (!stories.stories) {
        ...
        return (
            <div id="story-list">
                ...
                {filterStories().length > 0 ? filterStories().map(story => <div><Story story={story}/><button key={"edit-"+story.storyID}><a href={'/story/'+ story.storyID}>Edit Details</a></button></div>) : <div id='no-story-warning'>There are no stories to view from this date</div>}
                <div id="filter-archive">
                    <button onClick={()=> setArchiveFilter('Fanfiction.Net')}>Fanfiction.Net</button>
                    <button onClick={()=> setArchiveFilter('Archive of our Own')}>Archive of our Own</button>
                    <button onClick={()=> setArchiveFilter('All')}>All</button>
